...
class M_guru extends CI_Model{
	...
	function get_all_guru(){
		$hsl=$this->db->query("SELECT * FROM tbl_guru order by guru_nip ASC");
		return $hsl;
	}
	...
	//front-end
	function guru(){
		$hsl=$this->db->query("SELECT * FROM tbl_guru ORDER BY guru_nama DESC");
		return $hsl;
	}
	function guru_perpage($offset,$limit){
		$hsl=$this->db->query("SELECT * FROM tbl_guru ORDER BY guru_nama ASC limit $offset,$limit");
		return $hsl;
	}
